...
class MainApp(tk.Tk):
    ...
    def __init__(self, *, app_state: AppState, title="Sensor Application"):
        ...
        self.sensor_selection_notebook = SensorSelectionNoteBook(
            self, app_state=self.app_state
        )
        self.sensor_selection_notebook.grid(
            row=0, column=0, padx=10, pady=10, sticky="nsew"
        )
        self.app_state.attach(observer=self.sensor_selection_notebook)

        self.sensor_id_test_notebook = SensorIdTestNoteBook(
            self, app_state=self.app_state
        )
        self.sensor_id_test_notebook.grid(
        ...
        self.sensor_settings_frame.grid(
